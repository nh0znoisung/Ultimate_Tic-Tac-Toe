...
def train_nn(nn, game_mem):
    ...
    nn.fit(state, [policy, value], batch_size=16, epochs=training_epochs, verbose=1)
...
def pit(new_nn):
    ...
    for i in range(n_pit_network):
        ...
        while True:
            ...
            if len(possibleA) == 0:
                break
            action = random.choice(possibleA)
            next_state, mini_board, win = move(s, action, 1)
            s = next_state
            if win:
                print("Random attack wins")
                nn_wins +=1
                break

            # NN makes move, second turn
    ...
    print("NN_MCTS win: ", new_nn_wins)
    print("Random win: ", nn_wins)
    print("Tie match: ", n_pit_network - new_nn_wins - nn_wins)
    print("Win percent: ", win_percent)

    return int(win_percent * 100), new_nn_wins * 3 + (n_pit_network - new_nn_wins - nn_wins)
...
def train():
    ...
    if os.path.exists(os.path.join(save_model_path,'variables.pkl')):
      ...
      input_dictionary = {'Q':Q, 'Nsa':Nsa, 'Ns':Ns, 'W':W, 'P':P} #dictionary of tuple
    ...
    for episode in range(train_episodes):
        ...
        game_mem = []
        print("~~~~~ Test game with old nn | {} games ~~~~~".format(n_pit_network))

        win_rate, score = pit(nn) # Test game more faster

        nn.save(os.path.join(save_model_path,'model_{}_{}.h5'.format(win_rate,score)))  
        nn.save(os.path.join(save_model_path,'temp.h5'))  

        print("The global variables updating")
        input_dictionary = {'Q':Q, 'Nsa':Nsa, 'Ns':Ns, 'W':W, 'P':P} #dictionary of tuple
        file = open(os.path.join(save_model_path,'variables.pkl'), 'wb')
        pickle.dump(input_dictionary, file)
        file.close()
